...
function getFeatureRows(): HTMLElement[] {
  return Array.from(document.querySelectorAll<HTMLElement>('div.group')).filter((el) =>
    el.className.includes('justify-between') &&
    el.textContent?.trim().length
  );
}

function collectSettingsFromFeatureRows(): Record<string, boolean> {
  ...
  for (const row of getFeatureRows()) {
    ...
    const enabled = knob?.className.includes('bg-primary') ?? true;
